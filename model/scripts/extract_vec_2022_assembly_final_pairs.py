...
def elected_member(source: str, district: str, candidates: list[str]) -> tuple[str, str]:
    ...
    for candidate in candidates:
        match = re.search(r"Elected member\s+" + re.escape(candidate) + r"\s+(.+?)\s+Recheck votes", text, flags=re.I)
        if match:
...
def build(cache_dir: Path, refreshed_manifest: Path | None = None, refresh_output: Path | None = None, workbook_dir: Path | None = None, workbook_index: Path | None = None) -> list[dict[str, object]]:
    ...
    pages = [row for row in manifest if row["election_year"] == "2022" and row["contest"] == "general-election" and row["district_name"] != "[summary]"]
    if len(pages) != 87 or any(row["district_name"] == "Narracan" for row in pages):
        raise ValueError("VEC 2022 general-election source universe must be exactly 87 districts without Narracan")
    ...
    for source in pages:
        ...
        if len(finalists) != 2:
            link = re.search(r'href=["\']([^"\']*results-distribution)["\']', text, flags=re.I)
            if link:
        ...
        results.append({
            "election_id": "vic_la_2022", "district_id": re.sub(r"[^a-z0-9]+", "-", district.casefold()).strip("-"), "district_name": district,
    ...
    if len({row["district_id"] for row in results}) != 87:
        raise ValueError("duplicate or missing 2022 Assembly district IDs")
...
def main() -> None:
    ...
    rows = build(args.cache_dir, args.refreshed_manifest, args.refresh_manifest, args.workbook_dir, args.workbook_index)
    fields = list(rows[0])
    args.output.parent.mkdir(parents=True, exist_ok=True)
    with args.output.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader(); writer.writerows(rows)
    print(json.dumps({"path": str(args.output.resolve().relative_to(REPO)), "rows": len(rows), "sha256": hashlib.sha256(args.output.read_bytes()).hexdigest(), "narracanIncluded": False, "finalPairUnavailable": sum(1 for row in rows if not row["final_pair_available"])}))
