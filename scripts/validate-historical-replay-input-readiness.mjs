...
export function validateHistoricalReplayInputReadiness() {
  ...
  if (readiness.cycles.length !== 4 || readiness.runnableCycles !== 1 || readiness.predictedCycles !== 1 || readiness.scoredCycles !== 1 || readiness.archiveCompleteIsNotRequiredForRunnable !== true) throw new Error("historical replay readiness: cycle inventory is unsafe");
  ...
  if (generated.observations.length !== canonical.observations.length || observationsByCycle.vic_la_2018?.length !== 4 || observationsByCycle.vic_la_2022?.length !== 2 || canonical.independentSourceFamilies < sparse.minimumIndependentSourceFamilies) throw new Error("historical replay readiness: approved poll input inventory is inconsistent with governed cycle evidence");
  ...
  if (cycle2022?.pollEvidence.status !== "blocked" || !cycle2022.pollEvidence.reason.includes("fixed minimum") || !cycle2022.pollEvidence.reason.includes("Freshwater")) throw new Error("historical replay readiness: 2022 poll sufficiency blocker was not preserved");
