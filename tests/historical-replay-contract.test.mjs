...
test("frozen replay contract remains leakage-safe and fail-closed", () => {
  ...
  assert.equal(report.runnableCycles, 2);
  ...
  assert.ok(report.cycles.filter((cycle) => ["vic_la_2010", "vic_la_2014"].includes(cycle.id)).every((cycle) => cycle.blockedBy.length > 0));
});
