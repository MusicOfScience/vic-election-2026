...
test("fingerprints every available validation artefact and keeps partial evidence fail-closed", () => {
  ...
  assert.equal(pollQueue.coverage.sourceMatchedRows, 57);
  ...
  assert.equal(pollEvidence.sourceFamilies[1].matchedObservations.length, 24);
  assert.equal(pollEvidence.sourceFamilies[1].unresolvedObservations.length, 9);
  assert.equal(pollEvidence.sourceFamilies[1].coverage.explicitMethodRows, 23);
