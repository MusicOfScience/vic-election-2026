...
test("fingerprints every available validation artefact and keeps partial evidence fail-closed", () => {
  ...
  assert.equal(pollQueue.coverage.sourceMatchedRows, 56);
  ...
  assert.equal(pollEvidence.sourceFamilies[1].matchedObservations.length, 23);
  assert.equal(pollEvidence.sourceFamilies[1].unresolvedObservations.length, 10);
  assert.equal(pollEvidence.sourceFamilies[1].coverage.explicitMethodRows, 22);
