...
export function DataSources() {
  ...
  return <div className="tab-stack provenance-stack">
    ...
    <section className="release-finding" role="note" aria-label="Historical VEC evidence boundary"><Landmark size={17} /><div><strong>Historical VEC results are available—but they are outcomes</strong><p>The official results, boundaries and preference records anchor historical baselines and score held-out elections. They are never treated as polling that a past forecast could have known. Pre-election poll publication dates, candidate slates and preference evidence are tracked separately; the delayed Narracan supplementary result stays outside the November 2022 cycle.</p></div></section>
    ...
    <section className="surface replay-readiness" aria-label="Historical replay readiness">
      <div className="section-heading"><div><p className="eyebrow">Validation engine</p><h3>Historical replay readiness</h3><p className="section-subcopy">Each cycle is evaluated against its frozen cutoff; runnable validation is separate from production authorisation.</p></div><Badge variant="outline">{replayCycles.filter((cycle) => cycle.runnable).length}/{replayCycles.length} cycles runnable</Badge></div>
      <div className="replay-cycle-list">{replayCycles.map((cycle) => <article key={cycle.id}><div><strong>{cycle.id.replace("vic_la_", "Victorian ")}</strong><span>Information cutoff {readableDate(cycle.informationCutoff)}</span></div><span className="replay-status">{cycle.runnable ? "Runnable for validation" : "Not runnable"}</span><ul>{cycle.blockedBy.map((blocker) => <li key={blocker}>{blocker.replaceAll("-", " ")}</li>)}</ul></article>)}</div>
    </section>
    <section className="surface release-ledger">
      ...
      <div className="release-finding candidate-finding" role="note"><Vote size={17} /><div><strong>Candidate evidence—not official nomination</strong><p>{releaseReadiness.findings.candidateDiscovery.label}, drawn from {releaseReadiness.findings.candidateDiscovery.acceptedRecords} reviewed records across {releaseReadiness.findings.candidateDiscovery.sourceFamilies} source families. All 88 Assembly districts and all {releaseReadiness.findings.candidateDiscovery.councilRegions} Council regions now have evidence coverage; only the VEC may confer official nomination status.</p></div></div>
...
export function ElectionDashboard() {
  return <main className="site-shell">
    ...
    <header className="topbar"><div className="topbar-inner"><a href="#content" className="brand"><span className="brand-mark">V</span><span><strong>Victorian Election</strong><small>Forecasting laboratory · 2026</small></span></a><div className="status-cluster" aria-label={`Experimental forecast snapshot from ${forecastDate}`}><span className="status-dot" aria-hidden="true" /><span>Experimental snapshot · {forecastDate}</span></div></div></header>
    <div className="content-shell" id="content"><section className="page-heading"><div><p className="eyebrow">The election, rendered honestly</p><h1>A forecast you can interrogate.</h1><p className="page-deck">See the headline, then open the evidence: polls, all 88 electorates, Upper House regions, historical swings, source provenance and the tests the model passed—or failed.</p></div><div className="snapshot-stamp"><span>Snapshot date</span><strong>{forecastDateUpper}</strong><span>Model runs</span><strong>{number.format(modelOutput.manifest.simulations)} WHOLE ELECTIONS</strong></div></section>
