...
- Branch: `codex/rebuild-2014-crosswalk`
- Base commit: `c4aa75dd` (PR #109 merged; remote main synchronised). The 2022 certifying prediction and score are immutable.
- Current batch: locate or deterministically rebuild the original prediction-safe 2014 multi-party geographic crosswalk.
...
- Local/Council state: the 88-row ABC/Antony Green notional-margin baseline and all eight new-boundary Council priors remain governed. The ABC 2014-11-13 nominations summary supports the 88-row ballot mask at `model/data/validation/historical-replay-2014-ballot-mask.csv` (SHA `63156559…`); One Nation remains verified unavailable. The original four raw crosswalk inputs are absent from the bounded local/project search, Git history, branches, LFS and manifests; the source audit is `metadata/historical-replay-2014-crosswalk-source-audit.json`, so Assembly local readiness remains blocked. Council no longer needs a region-name continuity join.
- Publication: PR #109 (`https://github.com/MusicOfScience/vic-election-2026/pull/109`) is merged at `c4aa75dd`; this batch is prepared on `codex/rebuild-2014-crosswalk`. Local checks are green; GitHub Actions may remain externally blocked by the account billing/spending-limit restriction. Do not merge or deploy manually.
...
- The remaining 2014 blocker is the prediction-safe multi-party geographic translation/crosswalk. The four original raw inputs are all missing locally: the 2011 BCP/SA1 payload, 2011 Victorian mesh-block shape, 2011 census-count CSV, and 2013 Assembly boundary shape. No substitute or target-outcome-derived source is authorised. Ballot availability is closed with the cutoff-safe 88-row family mask; Council and preference inputs retain their governed broad fallbacks.

Exact next action: obtain the four original governed raw source payloads, verify their complete SHA-256 values and duplicate identity, then reproduce the fractional mesh-block/district bridge. Do not generate a prediction until the incumbency/local gate passes.
