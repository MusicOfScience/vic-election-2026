...
`metadata/model-validation-evidence-contract.json` records the evidence required to test the complete system. The existing four-cycle TPP residual ledger satisfies the two-party benchmark component only. Leakage-safe specifications are frozen for 2010, 2014, 2018 and 2022, but they remain deliberately unrunnable. A version-pinned candidate polling series now establishes 200 observations across the four cycles, yet it is quarantined because fieldwork midpoint cannot substitute for publication date and sample, explicit method, observation-level source and reuse-licence fields are absent. Party-primary/ballot, preference/final-pair and Council evidence also remains partial.

`metadata/model-validation-evidence-inventory.json` is the fingerprinted, machine-audited account of progress against that contract. It currently records one complete component and six partial components, with no component wholly missing. The partial evidence includes the quarantined 200-row polling candidate series; complete 2010 Assembly primary and ballot records for 502 candidates across all 88 districts; 323 audited candidate rows across 39 districts in 2022; 39 indicative Assembly preference distributions; high-level anchors for all eight 2022 Council regions; and four deterministic historical cycle specifications. The specifications enforce election-eve cutoffs, scoring-only outcomes and zero weight for the rejected demographic challenger. Partial is not complete: these artefacts do not open either complete-model gate.
...
`metadata/historical-public-source-audit.json` and `metadata/historical-source-acquisition-plan.json` enforce a supplied-and-first-party-source-first workflow. The supplied 1,088-page VEC 2010 report now yields a fingerprinted, fully reconciled 502-candidate outcome table for all 88 districts and a 440-row five-family scoring target. The VEC's public 2014, 2018 and 2022 district sources must be harvested before any VEC request; the 39 processed indicative preference distributions are not misrepresented as the limit of published 2022 primary data. The frozen 200-row polling file remains a lead list while original pollster and publisher records are reconstructed. External contact is deferred until a machine-readable residual-gap report exists, and no action is currently required from the project owner. Deferred fallback wording is maintained in `docs/HISTORICAL_SOURCE_REQUESTS.md`. All acquired material remains quarantined until every acceptance rule passes.
