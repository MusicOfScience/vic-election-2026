...
`metadata/model-validation-evidence-contract.json` records the evidence required to test the complete system. The four-cycle TPP residual ledger satisfies the two-party benchmark component, and the official Assembly primary outcome component is now complete. Leakage-safe specifications are frozen for 2010, 2014, 2018 and 2022, but they remain deliberately unrunnable. A version-pinned candidate polling series establishes 200 observations across the four cycles, yet it is quarantined because fieldwork midpoint cannot substitute for publication date and sample, explicit method, observation-level source and reuse-licence fields are absent. Ballot timing/incumbency, preference/final-pair and Council evidence remain partial.

`metadata/model-validation-evidence-inventory.json` is the fingerprinted, machine-audited account of progress against that contract. It records two complete components and five partial components, with no component wholly missing. Official Assembly primary outcomes comprise 2,296 candidate rows and 1,760 five-family rows across all 352 district-cycle contests. The partial evidence includes the quarantined 200-row polling candidate series; final ballot identities whose pre-cutoff timing and incumbent/challenger states remain unresolved; 39 indicative Assembly preference distributions; and official Council candidate primaries plus complete published count-total sequences for all 32 region-cycle contests in 2010, 2014, 2018 and 2022. Across all four cycles, the Council evidence contains 1,394 candidate rows, 5,975 count events, 296,087 count-total rows and 160 elected candidates; only cycle-specific rule configuration remains unresolved. Four deterministic historical cycle specifications enforce election-eve cutoffs, scoring-only outcomes and zero weight for the rejected demographic challenger. Partial is not complete: these artefacts do not open either complete-model gate.
