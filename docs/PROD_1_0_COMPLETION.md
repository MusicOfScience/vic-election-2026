...
| Runnable frozen historical configurations | Partial | Four specifications exist but are deliberately unrunnable | Complete frozen inputs and scoring-only outcome separation | Python model tests plus a replay command | Open blocker |
| Complete multi-party walk-forward backtest | Blocked | Existing validation is a two-party baseline, not the released probability model | Implement leakage-safe four-cycle replays and preregister acceptance criteria | New replay/backtest checks | Open blocker |
| Probability calibration | Blocked | No held-out complete-model reliability, slope/intercept or coverage evidence | Evaluate out-of-fold probabilities and intervals; retain honest limitations | New calibration checks | Open blocker |
